...
const Navigation: React.FC<NavigationProps> = ({ selectedMenu = "character", className = "" }) => {
  ...
  return (
    ...
    <Tabs defaultValue={selectedMenu} onValueChange={handleTabChange} className={`w-full ${className}`}>
      <TabsList className="
          is-rounded-navi
          flex
          relative
              ...
              <span className="pt-1 text-xs min-[430px]:text-sm">
                {(Array.isArray(label) ? label : [label]).map((text, brIndex) => (
                  <React.Fragment key={brIndex}>
                    {text}
                    {brIndex !== (Array.isArray(label) ? label.length - 1 : 0) && <br className="hidden max-[430px]:block" />}
                  </React.Fragment>
                ))}
              </span>
            </TabsTrigger>
